						...
						<c:if test="${!empty model.encounterListForChart}">
							...
							<c:forEach var="enc" items="${model.encounterListForChart}">
								<option value="${enc.encounterId}">
									<openmrs:formatDate date="${enc.encounterDatetime}"/> / ${enc.getActiveEncounterProviders().iterator().next().getProvider().getPerson().getFamilyName()} ${enc.getActiveEncounterProviders().iterator().next().getProvider().getPerson().getGivenName()} / (${enc.location})
								</option>	
							</c:forEach>
							</select></td></tr>
						</c:if>
